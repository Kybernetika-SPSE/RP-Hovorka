...
ESP32-P4 je vybaven vícejádrovou architekturou, která umožňuje paralelní zpracování úloh, což je klíčové zejména při práci s obrazovými daty. Dále disponuje hardwarovou akcelerací pro vybrané operace, dostatečnou kapacitou paměti a podporou vysokorychlostních rozhraní CSI, DSI a QSPI která umožňují efektivní přenos dat z kamerových modulů.
...
\begin{figure}
    ...
    \label{fig:esp32p4}
\end{figure} 
Sám o sobě ale být použit nemůže. Pro jeho správnou funkci je potřeba externí flash pamět, externí krystal a samostatně řešené napájení jednotlivých domén, včetně větve pro high-power piny. Bez správného napájení a startovacích podmínek může čip sice být osazený, ale zařízení se po zapnutí nemusí vůbec spolehlivě rozběhnout.
...
Na obrázku~\ref{fig:i2c_bootstrappy} je zachycena nejdůležitější podpůrná část okolo ESP32-P4. Je na ní vidět dvojice pull-up rezistorů pro sběrnici I2C, protože tato sběrnice je otevřenokolektorová a bez externího přitažení na 3,3 V by se signály nevytvářely správně. Dále je zde resetovací tlačítko a kapacita pro filtraci stisku, takže reset není jen „kontakt na zem“, ale krátký a stabilní impuls bez falešných přechodů.

Stejně důležitá je i pravá část obrázku, kde jsou zakreslené startovací a strapping vazby. Ty určují, v jakém režimu čip po zapnutí naběhne, a proto musí být v klidovém stavu jednoznačně definované. Pokud by byly ponechané bez pull-upů nebo pull-downů, mohl by se procesor po resetu chovat nespolehlivě, bootovat do nesprávného režimu nebo vůbec nenastartovat.

\begin{figure}
    \centering
    \includegraphics[width=0.95\linewidth]{pictures/i2c_bootstrappy_.png}
    \caption{Kritické pomocné zapojení kolem ESP32-P4: I2C pull-upy, reset a startovací vazby. \textit{[Zdroj: Vlastní]}}
...
\begin{figure}
    ...
    \caption{Zapojení konektoru pro připojení kamerového modulu IMX219. \textit{[Zdroj: Vlastní]}}
\end{figure}

Text této části je záměrně veden obecně na úrovni použitého rozhraní a zapojení. Konkrétní parametry obrazového snímače je vždy nutné vztáhnout k reálně osazenému kamerovému modulu. Je tedy možné použít kamerový modul s stjným pinoutem, ale s jinými parametry, a to i bez nutnosti úprav v zapojení. Naopak použití modulu s jiným pinoutem by vyžadovalo zásadní úpravy v zapojení i v softwaru.
...
\begin{figure}
    \centering
    \includegraphics[width=0.7\linewidth]{pictures/sdkarta.png}
